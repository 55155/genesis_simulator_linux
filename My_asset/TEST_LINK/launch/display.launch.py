...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='TEST_rigidbody_description').find('TEST_rigidbody_description')
    default_model_path = os.path.join(pkg_share, 'urdf/TEST_rigidbody.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/TEST_rigidbody/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'TEST_rigidbody', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
